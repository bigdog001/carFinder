...
//	fd[camid] = open (dev_name[camid], O_RDWR );
...
int initdevice(int i)
{
	...
	fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV;	//V4L2_PIX_FMT_YUYV,V4L2_PIX_FMT_MJPEG,V4L2_PIX_FMT_JPEG,V4L2_PIX_FMT_MPEG
	fmt.fmt.pix.field       = V4L2_FIELD_INTERLACED;

	if (-1 == xioctl (fd[i], VIDIOC_S_FMT, &fmt)){
		LOGD(" does not support %s:",V4L2_PIX_FMT_MJPEG);
...
{
	yuv422_to_rgb(rgb[i],p,IMG_WIDTH,IMG_HEIGHT);
}

void processimage_jpg (int i, const void *p, int l)
{
...
jint Java_com_usb_camera_android_CameraUtil_prepareCamera(JNIEnv* env,jobject thiz, jint videoid){

	int ret_one,ret_all=0;
	int i;
	...
	for(i=0 ; i<CAMERANUM ; i++){
		ret_one = opendevice(i, camerabase+videoid+i);
		LOGI("opendevice ret:%d",ret_one);
		if(ret_one  != ERROR_LOCAL){
			ret_one  = initdevice(i);
			LOGI("initdevice ret:%d",ret_one);
		}
		if(ret_one  != ERROR_LOCAL){
			ret_one = startcapturing(i);
			LOGI("startcapturing ret:%d",ret_one);
		}
		...
		if(ret_one  != ERROR_LOCAL){
			...
			close(fd[i]);//如果没能通过测试，则前面打开的描述符作废。
			fd[i]=-1;
		}
	}
	LOGI("ret_all:%x",ret_all);
	return ret_all;
}	

jint Java_com_usb_camera_android_CameraUtil_prepareCameraWithBase( JNIEnv* env,jobject thiz, jint videoid, jint videobase){
	
	int ret;

	camerabase = videobase;

	return Java_com_usb_camera_android_CameraUtil_prepareCamera(env,thiz,videoid);

}

void Java_com_usb_camera_android_CameraUtil_processCamera( JNIEnv* env,jobject thiz){
...
void Java_com_usb_camera_android_CameraUtil_stopCamera(JNIEnv* env,jobject thiz){
...
void Java_com_usb_camera_android_CameraUtil_processRBCamera( JNIEnv* env,jobject thiz, jint lrmode){
	int i;
	int red = 1-lrmode;
	int blue = lrmode;
...
jint Java_com_usb_camera_android_CameraUtil_checkCamera( JNIEnv* env,jobject thiz){

}
void Java_com_usb_camera_android_CameraUtil_takeImage( JNIEnv* env,jobject thiz,jobject bitmap1, jobject bitmap2,jint videoflag){
